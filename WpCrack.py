...
def login(url, username, password, timeout, proxy):
    ...
    form = "log={}&pwd={}&wp-submit=Acceder&redirect_to={}/wp-admin&testcookie=1".format(username, password,url_abs)
    ...
    try:
        ...
        with urllib.request.urlopen(request, timeout=timeout, context=context) as response:
            print (f'response.url {response.url} username {username} password {password}')
